...
public class Apriori extends Observable { 
    ...
    }      private List<int[]> itemsets ;     private String transaFile;      private int numItems;      private int numTransactions;      private double minSup;  
    private boolean usedAsLibrary = false; 
    public  Apriori(String[] args, Observer ob) throws Exception 
    {      usedAsLibrary = true;      configure(args);      this.addObserver(ob); 
     go();     } 
    public  Apriori(String[] args) throws Exception 
    { 
        configure(args);         go();     } 
    private void go() throws Exception {         long start = System.currentTimeMillis();         createItemsetsOfSize1();                 int itemsetNumber=1;          int nbFrequentSets=0; 
...
" + itemsetNumber + " (with support "+(minSup*100)+"%)");;                 createNewItemsetsFromPreviousOnes(); 
    ...
    {        
     ...
     if (args.length>=2) 
minSup=(Double.valueOf(args[1]).doubleValue());     	      else minSup = .8;// by default 
     if (minSup>1 || minSup<0) throw new Exception("minSup: bad value"); 
           numItems = 0;      numTransactions=0; 
     BufferedReader data_in = new BufferedReader(new 
FileReader(transaFile)); 
     while (data_in.ready()) {     	 	       	String line=data_in.readLine(); 
 	...
 	private void outputConfig() {  	 	 log("Input configuration: "+numItems+" items, 
    ...
    { 
     ...
     HashMap<String, int[]> tempCandidates = new HashMap<String, int[]>(); – 
        ...
        { 
            ...
            { 
                int[] X = itemsets.get(i);                 int[] Y = itemsets.get(j); 
                ...
                { 
                    ...
                    for(int s2=0; s2<X.length; s2++) {                      if (X[s2]==Y[s1]) {                        	found = true; 
                      	break; 
                     }                  	} 
                ...
                if (ndifferent==1) {                  	Arrays.sort(newCand); 
...
InputStreamReader(new FileInputStream(transaFile))); 
 	 	...
 	 	extractEncoding1(data_in.readLine());  	 	String line = data_in.readLine();  	 	 	line2booleanArray(line, trans);  	 	 	for (int c = 0; c < itemsets.size(); c++) {  	 	 	 	match = true; // reset match to false  	 	 	 	int[] cand = itemsets.get(c);  	 	 	 	for (int xx : cand) {  	 	 	 	 	if (trans[xx] == false) { 
 	 	 	 	 	 	match = false; 
 	 	 	 	 	 	break; 
 	 	 	 	 	} 
 	 	 	 	} 
